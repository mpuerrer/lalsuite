...
struct tagSurrogatedata_submodel
{
  ...
  gsl_vector *TF2_mf_amp;        // location of spline nodes TaylorF2 amplitude at low frequency
  gsl_vector *TF2_mf_phi;        // location of spline nodes TaylorF2 phasing at low frequency
...
static void Surrogatedata_Cleanup_submodel(Surrogatedata_submodel *submodel) {
  ...
  if(submodel->TF2_mf_amp) gsl_vector_free(submodel->TF2_mf_amp);
  if(submodel->TF2_mf_phi) gsl_vector_free(submodel->TF2_mf_phi);
  ...
  )
{
  ...
  retcode |= TaylorF2Amplitude1PN(eta, sur->TF2_mf_amp, &TF2_amp_at_nodes);
